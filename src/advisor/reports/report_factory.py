...
class ReportFactory:
    ...
    def createReport(self, root_directory, branch, savings, productname, teamname, target_os='linux', issue_type_config=None, output_format=ReportOutputFormat.TEXT):
        match output_format:
            case ReportOutputFormat.TEXT:
                report = TextReport(root_directory, target_os=target_os)
            case ReportOutputFormat.HTML:
                report = HtmlReport(root_directory, branch, savings, productname, teamname, target_os=target_os)
